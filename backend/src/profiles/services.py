...
class ProfileService:
    ...
    async def get_by_id(self, profile_id: int) -> ProfileFromDB | None:
        profile = await self._repo.get(profile_id)
        if profile:
            return ProfileFromDB.model_validate(profile)
        return None

    async def add(self, schema: ProfileSchema) -> int:
        return await self._repo.add(schema.model_dump(exclude_none=True))

    async def get_by_user_id(self, user_id: int) -> ProfileFromDB | None:
        profile = await self._repo.get_by_user_id(user_id)
        if profile:
            return ProfileFromDB.model_validate(profile)
        return None

    async def update_by_user_id(self, user_id: int, schema: ProfileUpdate) -> ProfileFromDB | None:
